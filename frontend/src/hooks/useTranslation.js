...
export function useTranslation(t) {
  ...
  const [targetLang, setTargetLangState] = useState('pt');
  const [translateStructuralTitles, setTranslateStructuralTitles] = useState(true);
  ...
  const oppositeLanguage = (value) => value === 'en' ? 'pt' : 'en';

  const buildTranslateParams = () => {
    const params = new URLSearchParams();
    if (limitPages) {
      params.set('limit_pages', limitPages);
    }
    params.set('source_lang', sourceLang);
    params.set('target_lang', targetLang);
    params.set('translate_structural_titles', translateStructuralTitles ? 'true' : 'false');
    params.set('enable_ai_rewrite', enableAiRewrite ? 'true' : 'false');
  ...
  const handleSourceLangChange = (value) => {
    ...
    if (value === targetLang) {
      setTargetLangState(oppositeLanguage(value));
    }
  };
  ...
  const handleTargetLangChange = (value) => {
    ...
    if (value === sourceLang) {
      setSourceLangState(oppositeLanguage(value));
    }
  };
  ...
  const resetAll = () => {
    ...
    setTargetLangState('pt');
    setTranslateStructuralTitles(true);
    setEnableAiRewrite(false);
  };
  ...
  return {
    ...
    limitPages, sourceLang, targetLang, translateStructuralTitles, enableAiRewrite,
